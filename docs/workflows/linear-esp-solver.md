...
The linear solver fits electrostatic potential (ESP) charges that reproduce grid values exported by RESP/TeraChem. Everything lives in `protocharge.linearESPcharges.linear` and is backed by numpy.

## Pipeline

1. **Parse Terachem RESP output** with `ParseRespDotOut` (from `protocharge.resp_parser`) to obtain atomic positions and RESP ESP charges for each frame.
