...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate two-step RESP charges for all configurations of a microstate (mask-aware total constraint)."
    )
    parser.add_argument(
        "--microstate-root",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="Output NPZ path (default: results/<microstate>/twostepRESP_masked_total/charges_constraint.npz).",
    )
    parser.add_argument(
        "--bucket-file",
        type=Path,
...
def main() -> None:
    ...
    output_path = (
        args.output
        or (
            ensure_results_dir(microstate_root.name, "twostepRESP_masked_total")
    ...
    np.savez_compressed(
        ...
        total_charge=total_charge_target,
        total_constraint_labels=np.asarray(total_constraint_labels, dtype=object),
    )
