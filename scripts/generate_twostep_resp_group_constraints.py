...
from protocharge.training.twostepresp_group_constraints.tsresp import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate two-step RESP charges for all configurations (group constraints by index)."
    )
    parser.add_argument(
        "--microstate-root",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="Output NPZ path (default: output/<microstate>/twostepRESP_group_constraints/charges_constraint.npz).",
    )
    parser.add_argument(
        "--bucket-file",
        type=Path,
    ...
    parser.add_argument(
        "--group-constraint",
        type=Path,
        help="Override group constraint YAML (default: <microstate-root>/charge-contraints/group_constraint.yaml).",
    )
    parser.add_argument(
        "--bucket-constraints",
        type=Path,
...
def main() -> None:
    ...
    for idx, (stem, resp_path, esp_path) in enumerate(configs, start=1):
        ...
        if np.any(mask_step2):
            ...
            constraint_adjusted = d_vector - (C @ p_fixed)
    ...
    output_path = (
        args.output
        or (
            ensure_results_dir(microstate_root.name, "twostepRESP_group_constraints")
