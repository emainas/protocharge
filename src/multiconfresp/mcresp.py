...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--save",
        action="store_true",
        help="Persist the stacked Coulomb matrix and ESP vector under data/microstates/<microstate>/multiconfRESP/.",
    )
    parser.add_argument(
        "--load-and-resp",
        action="store_true",
...
def _microstate_root(project_root: Path, microstate: str) -> Path:
    root = project_root / "data" / "microstates" / microstate
    if not root.is_dir():
...
def _save_stacked_matrices(
    ...
    target_dir = microstate_root / "multiconfRESP"
...
def _run_resp_from_saved(
    ...
    *,
    ...
    total_constraint_path = microstate_root / "charge-contraints" / "total_constraint.yaml"
    ...
    bucket_constraint_path = microstate_root / "charge-contraints" / "bucket_constraints.yaml"
    ...
    mask_step1_path = microstate_root / "charge-contraints" / "mask_step_1.yaml"
    mask_step2_path = microstate_root / "charge-contraints" / "mask_step_2.yaml"
